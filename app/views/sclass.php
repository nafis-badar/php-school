...
<div class="workspace">
  <div class="displayhead">Class/Section<span style=""  name="Operation"   class="btn btn-primary pull-right"  onClick="SaveSClass();" > <i class="fa fa-check-circle-o" aria-hidden="true"></i> Save </span> <span style="margin-right:5px;"  name="Operation"   class="btn btn-primary pull-right " onClick="redirectBypagewithIDValue('menu',0,0);" > Dashboard</span> </div>
...
if($ID!=0)
{
	...
	//echo $sql;
$getinfo = $Handle->SClassDetails($ID);
...
for ($x = 0; $x < count($getinfo,0); $x++)
	{
		...
		$Capacity=$getinfo[$x]["Capacity"];
		$ClassTeacherID=$getinfo[$x]["ClassTeacherID"];
    ...
    <div class="con-hr">
      ...
      <input type="hidden" id="DrCr" value="<?php echo $DrCr; ?>">
      <input type="hidden" id="MasterGroupID" value="<?php echo $MasterGroupID; ?>">
    ...
    <div class="form-group col-lg-3">
  <label class="control-label" for="CODE">Alias:</label>
  <div class="">
  <input type="text" class="form-control" id="CODE" name="CODE" placeholder="Enter CODE." value="<?php echo $CODE; ?>"> 
</div></div>
 <div class="form-group col-lg-3">
  <label class="control-label" for="Capacity">Capacity of Student:</label>
  <div class="">
  <input type="text" class="form-control" id="Capacity" name="Capacity" placeholder="Enter Capacity." value="<?php echo $Capacity; ?>"> 
</div></div>
        <div class="form-group col-lg-3 ">
      <label for="MID" class="control-label"  >Class:</label>
      <div class="">
        <select name="MID" id="MID" class="form-control" >
          <option value="0">Please select...</option>
          <?php
$getitemgroupinfo = $Handle->runQuery("SELECT ID,Name FROM tbl_master where MasterTypeID=44 and MID=0  order by Name");
$GroupID=$MID;
for ($ig = 0; $ig < count($getitemgroupinfo,0); $ig++) {
    ...
    <div class="col-lg-12"> 
      <div class="form-group col-lg-3 ">
      <label for="ClassTeacherID" class="control-label"  >Class Teacher:</label>
      <div class="">
        <select name="ClassTeacherID" id="ClassTeacherID" class="form-control" >
          <option value="0">Please select...</option>
          <?php
$getSinfo = $Handle->runQuery("SELECT ID,Name FROM tbl_master where MasterTypeID=43  order by Name");
//$GroupID=$MID;
for ($is = 0; $is < count($getSinfo,0); $is++) {
       ?>
          <option value="<?php echo $getSinfo[$is]['ID'];?>" <?php if($ClassTeacherID==$getSinfo[$is]['ID']) echo 'selected= "selected"'; ?>><?php echo $getSinfo[$is]['Name'];?></option>
          <?php } ?>
        </select>
      </div>
    </div>
...
function SaveCustomer0()
{
 ...
 function SaveSClass()
 {
	 	var pg='app/controllers/Save.SClass.php';
	 ...
	 var ClassTeacherID = $("#ClassTeacherID").val();
	  ...
	  //alert(Capacity);
	  if(MID==0){IsMaster=1;}else{IsMaster=0;}
	 //
	 var ID = $("#ID").val();
	...
	var DrCr = document.getElementById("DrCr").value;
                ...
                $.post( pg, 
				{Type:Type,MasterTypeID:MasterTypeID,CODE:CODE,  ID:ID, MID:MID, PDID:PDID, Name:Name, AddressID:AddressID, ContactNo: ContactNo, Address: Address, RegDate: RegDate, Email: Email, State: State, Country:Country, IsMaster:IsMaster,MasterGroupID:MasterGroupID,DrCr:DrCr,DepartmentID:DepartmentID, Capacity:Capacity, ClassTeacherID:ClassTeacherID})
                    .done(function( data ) {
						// alert(DrCr);
						document.getElementById("Report").innerHTML=data;
                        //$("#message").html(data);
