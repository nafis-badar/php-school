...
function CheckInvoice($BookingID,$PatientID)
{
	$db= new DBFunction();
	...
	$sql="SELECT  c.ID from tbl_cart c  
	 ...
	 where vm.ID=$BookingID";
	// echo $sql.'<br>';
	$CartInfo= $db_handle->runQuery($sql);
	if(count($CartInfo,0)>0)
	{
		$CartID=$CartInfo[0]['ID'];
		$sql="SELECT * from tbl_cart_details where CartID=$CartID";
		 //echo $sql.'<br>';
		$CartDetailsInfo= $db_handle->runQuery($sql);
		//	 echo $sql.'<br>';
		...
		for($x=0; $x<count($CartDetailsInfo,0); $x++)
		{
			...
			$Quantity=$CartDetailsInfo[$x]['Quantity_Billing'];
			$Rate=$CartDetailsInfo[$x]['Rate_Primary'];
			$Discount=$CartDetailsInfo[$x]['Discount_Percentage'];
            ...
            ?>
            "><?php echo $db->ItemName($ItemID);?></td>
...
where  (vm.ID=".$BookingID." or vm.RefVoucherID=".$BookingID.") ";
	      // echo $sql;
	   $getAccountinfo = $db_handle->runQuery($sql);
	   ...
	   for($a=0;$a<count($getAccountinfo,0);$a++)
	   {
			...
			else
			{
				...
				?>
                <tr class=" ">
            	    <td colspan="2" style="text-align:left;"> <?php //echo "&nbsp;&nbsp;&nbsp;Rupees. " . $db->convert_number($Balance). " only";?></td>
   ...
   if($Balance>0)
	...
	?>
        ...
        <td colspan="3" style="text-align:left;"> <?php echo "&nbsp;&nbsp;&nbsp;Rupees. " . $db->convert_number($NetReceipt). " only";?></td>
...
function CheckReportExistance($ReportID)
{
	...
	$sql="SELECT  ID from tbl_testreportmaster where ID=$ReportID";
	 //echo $sql.'<br>';
	$ReportExistanceInfo= $db_handle->runQuery($sql);
	if(count($ReportExistanceInfo,0)>0)
	{
		...
		$sql="SELECT * from tbl_testreportdetails where TestReportID=$TestReportID";
		 //echo $sql.'<br>';
		$ReportDetailsInfo= $db_handle->runQuery($sql);
		//	 echo $sql.'<br>';
...
//$company = $db_handle->runQuery("SELECT * FROM tbl_master WHERE ID=1");
$companyname=$db->GetName(1);

$CompantPDID=$db->GetPDID(1);

//echo $CompantPDID;
$CompanyAddressID=$db->GetAddressID($CompantPDID);
$companyconatct=$db->GetContact($CompantPDID);
//$companyaddressQuery= $db_handle->runQuery("SELECT a.Display FROM  tbl_pd p left outer join tbl_address a on a.ID=p.AddressID WHERE P.ID='".$CompantPDID."'");
$companyaddress=$db->GetAddress($CompanyAddressID);

$voucherdetails = $db_handle->runQuery("SELECT * FROM tbl_vouchermaster WHERE ID='".$BookingID."'");
...
//$amount=$voucherdetails[0]["Amount"];
 ...
 $RefDoctor=$db->GetName($RefDoctorID);

//echo   $ReportDate;
//echo $VoucherDate;
/*$itemid=$voucherdetails[0]["ItemID"];
$customerid=$voucherdetails[0]["CustomerID"];
$ItemDetails = $db_handle->runQuery("SELECT * FROM tbl_item WHERE ID='".$itemid."'");
$item=$ItemDetails[0]["Alias"];
$itemduration=$ItemDetails[0]["Description"];*/
//echo $item;
//$PatientByID = $db_handle->runQuery("SELECT * FROM tbl_master WHERE ID='".$PatientID."'");
$PatientPDID=$db->GetPDID($PatientID);
$PatientName=$db->GetName($PatientID);
//$PatientDetailsByPDID = $db_handle->runQuery("SELECT * FROM tbl_pd WHERE ID='".$PatientPDID."'");
$PatientAddressID=$db->GetAddressID($PatientPDID);

//$PatientAddressResult= $db_handle->runQuery("SELECT * FROM tbl_address WHERE ID='".$PatientAddressID."'");
$PatientAddress=$db->GetAddress($db->GetAddressID($PatientPDID));
$PatientGender=$db->GetGender($PatientPDID);
$PatientAge=$db->GetAge($PatientPDID);
$PatientContact=$db->GetContact($PatientPDID);
$CreatedByName=$db->getUserName($voucherdetails[0]["CreatedBy"]);
...
<div  class="main-body" style="padding-left:5%; text-align:center; font-size:12px;"><?php CheckInvoice($BookingID,$PatientID);?></div>
<footer>
